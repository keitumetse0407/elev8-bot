...
const FINAL_AGENT   = '/data/data/com.termux/files/home/elev8_agent_cli.py';
const AUTH_DIR       = '/data/data/com.termux/files/home/elev8_auth';
const SESSIONS_FILE  = path.join(FINAL_OUT, '.sessions.json');
...
function setSession(num, data) {
    ...
    fs.writeFileSync(path.join(FINAL_OUT, '.sessions.json'), JSON.stringify(sessions, null, 2));
}
...
function runAgent(service, data) {
    return new Promise((resolve, reject) => {
        ...
        exec(`python3 ${FINAL_AGENT} '${escaped}'`, { timeout: 120000 }, (err, stdout, stderr) => {
...
// ── CONFIG ─────────────────────────────────────────────────
// Set PAIRING_NUMBER env var to your WhatsApp number (e.g. 27799002951)
// on first run to get pairing code. Remove after paired.
const PAIRING_NUMBER = process.env.PAIRING_NUMBER || '';

// ── PATHS (Render-compatible) ──────────────────────────────
const IS_RENDER  = !!process.env.RENDER;
const RENDER_AUTH = '/opt/render/project/src/auth_info';
const FINAL_AUTH  = IS_RENDER ? RENDER_AUTH : AUTH_DIR;
const FINAL_OUT   = IS_RENDER ? '/opt/render/project/src/elev8_outputs' : OUTPUT_DIR;
const FINAL_AGENT = IS_RENDER
    ? '/opt/render/project/src/elev8_agent_cli.py'
    : FINAL_AGENT;

// ── START BOT ─────────────────────────────────────────────
loadSessions();
fs.mkdirSync(FINAL_OUT,  { recursive: true });
fs.mkdirSync(FINAL_AUTH, { recursive: true });

async function startBot() {
    const { state, saveCreds } = await useMultiFileAuthState(FINAL_AUTH);
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        } else if (qr && !PAIRING_NUMBER) {
            console.log('\n[BOT] Set PAIRING_NUMBER env var to get pairing code.\n');
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                console.log('[BOT] Logged out. Remove auth_info folder and restart.');
                process.exit(1);
            }
        }
    });
